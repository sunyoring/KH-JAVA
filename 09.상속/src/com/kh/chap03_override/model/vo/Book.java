...
public class Book {
	...
	// 2. equals 메소드의 내용을 주소값 비교가 아닌 실제 필드값들이 모드 같을경우 true가 반환되고 하나라도 틀릴 경우 false가
	// 되도록 정의

	@Override
	public boolean equals(Object obj) {
		// bk1.equals(bk2)
		// this(현재객체) vs obj(전달받은 비교대상)
		// 자료형 : Book Object

		if (!(obj instanceof Book))// obj가 Book로 형변환이 가능하지 않다면
			return false;

		Book other = (Book) obj; // obj타입을 Book 타입으로 강제 형변환, 상속구조에서는 클래스로 형변환이 가능하다.
		// bk1.equals(bk2)
		// this(현재객체) vs obj(전달받은 비교대상)
		// 자료형 : Book Book
	...
	public int hashCode() {
		...
		// 즉, 같은 문자열일 경우 해당문자열의 hashCode는 같도록 String에서 이미 재정의
